...
class Trainer():
    ...
    def train(self, 
              model: nn.Module,
              s_train_iter: DataLoader,
              t_train_iter: DataLoader,
              val_iter: DataLoader,
              trainset_size: int,
              batch_size: int,
        ...
        for epoch in range(self.config["epochs"]):
            for t_batch in tqdm.tqdm(t_train_iter):
                for s_batch in s_train_iter:
                    logit, loss = self.foward_calculator(model, t_batch, s_batch, device=self.config['device'], cuda=self.config['cuda'], class_balance=class_balance)
                    loss.backward()
                    ...
                    labels.append(t_batch[6])  # type: ignore

                    if step > 0 and step % self.config['print_every'] == 0:
                        print_logits = torch.cat(logits, dim=0)  # type: ignore
                        print_labels = torch.cat(labels, dim=0).to(self.config["device"])
                        print_loss, print_metrics = self.loss_fn(print_logits, print_labels), self.metrics_fn(print_logits,
                                                                                                              print_labels)  # type: ignore
    ...
    def evaluate(self, model: nn.Module, eval_iter: DataLoader, save_file: str = "", save_title: str = ""):
        ...
        for step, batch in enumerate(eval_iter):
            logit = self.foward_calculator(model, batch, device=self.config['device'], cuda=self.config['cuda'], evaluate=True)
            logits.append(logit)
            labels.append(batch[6])

        logits = torch.cat(logits, dim=0) # type: ignore
        labels = torch.cat(labels, dim=0).to(self.config['device'])
